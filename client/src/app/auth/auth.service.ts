...
export class AuthService {
  ...
  constructor(private httpClient: HttpClient, private debug: DebugUtil) {}
  ...
  login(credentials: Credentials): Observable<AuthResponse> {
    return this.httpClient
      .post<AuthResponse>(`${API}/auth/login`, credentials, this.httpOptions)
      .pipe(
        tap({
          next: (response) => {
            if (response && response.accessToken && response.refreshToken) {
              ...
              this.scheduleTokenRefresh(
                response.expiresIn * 1000 - this.FIVE_MINUTES
              );
            } else {
              this.debug.error('Token JWT não recebido', 'AuthService.login', {
                token: response,
              });
            }
          },
      ...
      .pipe(
        ...
        catchError((error) => {
          this.logout().subscribe({
            complete: () =>
              console.error('Erro ao realizar o Refresh Token', error),
          });
          return throwError(error);
        })
      );
  ...
  logout(): Observable<void> {
    ...
    if (refreshToken) {
      return this.httpClient
        .post<void>(`${API}/auth/logout`, { refreshToken }, this.httpOptions)
        .pipe(
          catchError((error) => {
            this.debug.error('Erro no logout', 'AuthService.logout', {
              error: error,
            });
            return of(undefined);
          }),
